...
export function io_clk_rvpipe_register(sim_p: Simulator): Simulator
{
    ...
    sim_p.behaviors['IO_IOW'] = {
        ...
        operation:   function (s_expr: string[]): void
        {
            ...
            const iocr_id = get_value(sim_p.states[s_expr[4]]);
            const iodr_id = get_value(sim_p.states[s_expr[5]]);
    ...
    sim_p.behaviors['IO_RESET'] = {
        ...
        operation:   function (): void
        {
            ...
            const io_int_factory = sim_p.internal_states.io_int_factory;
            if (io_int_factory)
            {
                for (let i = 0; i < io_int_factory.length; i++)
                {
