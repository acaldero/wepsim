...
import { set_jit_fire_dep } from "./sim_hw_signal.js";
import { simhw_sim_signals, simhw_sim_states, simhw_sim_signal, simhw_syntax_behavior, simhw_sim_state } from "./sim_hw_index.js";
/*
 *  Behavior
 */
...
export function compile_behaviors() {
                ...
                if (s_expr[0].startsWith("FIRE")) {
                    ...
                    if (typeof expr_obj == "undefined") {
                        ...
                        if (s_expr[0] == "FIRE")
                            ...
                            jit_fire_dep[s_expr[1]][sig].fire_ifset++;
                    }
                }
            }
    ...
    // eval(jit_bes) ;
    let eval_executor = new Function('simhw_sim_signal', 'simhw_syntax_behavior', jit_bes);
    eval_executor(simhw_sim_signal, simhw_syntax_behavior);
    // update jit_fire_dep and jit_behaviors
    set_jit_fire_dep(jit_fire_dep);
    jit_behaviors = true;
}
...
export function compile_verbals() {
    ...
    // eval(jit_vbl) ;
    let eval_executor = new Function('simhw_sim_signal', 'simhw_syntax_behavior', jit_vbl);
    eval_executor(simhw_sim_signal, simhw_syntax_behavior);
    jit_verbals = true;
}
...
export function compute_signal_verbals(signal_name, signal_value) {
    ...
    if (jit_behaviors)
        ...
        verbal = compute_verbal(sig_ref.behavior[index]);
    return verbal;
}
