...
import { simhw_sim_ctrlStates_get, simhw_sim_state, simhw_internalState, simhw_sim_signals } from "../sim_hw/sim_hw_index.js";
...
export function hex2float(hexvalue) {
    ...
    if (-127 === exponent)
        if (1 === mantissa)
            ...
            valuef = sign * ((hexvalue & 0x7fffff) / 0x7fffff) * Math.pow(2, -126);
    if (128 === exponent)
        if (1 === mantissa)
...
export function value2string(format, value) {
    ...
    switch (fmt[0]) {
        ...
        case "float":
            ...
            break;
        case "char":
            fmt_value = "'" + String.fromCharCode(value) + "'"; // fmt[1] = ascii
            break;
