...
import { simhw_sim_ctrlStates_get, simhw_sim_state, simhw_sim_state_getref, simhw_internalState } from "../sim_hw/sim_hw_index.js";
...
export function main_memory_set(memory, elto, melto) {
    ...
    if (typeof melto.changed === "undefined")
        melto.changed = false;
    if (typeof melto.state === "undefined")
        melto.state = false;
    if (typeof melto.breakpoint === "undefined")
        melto.breakpoint = false;
    if (typeof melto.notify === "undefined")
        melto.notify = [];
    if (typeof melto.is_assembly === "undefined")
        melto.is_assembly = false;
    if (typeof melto.source === "undefined")
        melto.source = '';
    if (typeof melto.source_bin === "undefined")
        melto.source_bin = '';
...
export function main_memory_fusionvalues(dbvalue, value, filter) {
    ...
    else if (4 == (filter & 0x0000000C)) { // half
