...
export function update_cfg(field, value) {
    if (WSCFG[field].value != value) {
        simcore_ga('cfg', 'cfg.' + WSCFG.version.value, 'cfg.' + WSCFG.version.value + '.' + field + '.' + value);
    }
    WSCFG[field].value = value;
    // add if recording
    simcore_record_append_new('Set configuration option ' + field + ' to ' + value, 'ws.update_cfg("' + field + '","' + value + '");\n');
    save_cfg();
}
...
export function get_primary_cfg() {
    var wscfg = {
        /* version */
        "version": { upgrade: false, type: "string", value: "2.6.0" },
        "build": { upgrade: true, type: "string", value: "2.6.0.20260710A" },
...
export var cfg_show_rf_delay = 0;
export var cfg_show_eltos_delay = 0;
export var cfg_show_main_memory_delay = 0;
export var cfg_show_control_memory_delay = 0;
export var cfg_show_dbg_ir_delay = 0;
export var cfg_show_rf_refresh_delay = 0;
export var cfg_show_asmdbg_pc_delay = 0;
