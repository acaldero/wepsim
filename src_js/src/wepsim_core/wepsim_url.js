...
export function wepsim_file_saveTo(textToWrite, fileNameToSaveAs) {
    ...
    var koHandler = function (error) {
        wepsim_notify_error("<strong>ERROR</strong>: failed file write", "Failed file write. " +
            "Error found " + error.toString());
    };
    var okHandler = function (msg) {
        wepsim_notify_success('<strong>INFO</strong>', 'Successful file write request: ' + fileNameToSaveAs);
    };
    ...
    var onInitFs = function (fs) {
        fs.root.getFile(fileNameToSaveAs, { create: true, exclusive: false }, onCreatFile, koHandler);
    };
...
export function wepsim_file_loadFrom(fileToLoad, functionOnLoad) {
    ...
    fileReader.onerror = function (e) {
        wepsim_notify_error("<strong>ERROR</strong>", "File could not be read. " +
...
export function wepsim_file_downloadTo(textToWrite, fileNameToSaveAs) {
    ...
    downloadLink.click();
    wepsim_notify_success('<strong>INFO</strong>', 'Successful opportunity for downloading: ' + fileNameToSaveAs);
}
/*
 *  Auxiliar API
 */
    ...
    if (false === is_mobile()) {
        fetchURL(url).then(function (response) {
            if (typeof response == "undefined") {
                wepsim_notify_error("<strong>ERROR</strong>", "File " + url + " could not be fetched:<br>\n" +
