...
export function example_id2hash(example_id) {
    ...
    if (sid.length > 1)
        ...
        console.log("warning: example without microcode id\n" +
            " * example  id: " + example_id + "\n" +
            " * expected id: ep:*microcode_X*:assembly_Y\n");
    if (sid.length > 2)
        eltos.sample_asm = sid[2] + ".asm";
    else
        console.log("warning: example without assembly id\n" +
            " * example  id: " + example_id + "\n" +
            " * expected id: ep:microcode_X:*assembly_Y*\n");
    return eltos;
}
export function load_from_example_assembly(example_id, chain_next_step) {
    ...
    // add if recording
    simcore_record_append_new('Load assembly from example ' + example_id, 'ws.load_from_example_assembly("' + example_id + '", ' + chain_next_step + ');\n');
...
export function load_from_example_firmware(example_id, chain_next_step) {
    ...
    wepsim_load_from_url(url, do_next);
    // add if recording
    simcore_record_append_new('Load firmware from example ' + example_id, 'ws.load_from_example_firmware("' + example_id + '", false);\n');
    // stats about examples
...
export function share_example(m, base_url) {
    ...
    var share_url = '' + base_url + '?mode=' + e_hw +
        ...
        '&example=' + m;
    return share_information('example_' + m, share_title, share_text, share_url);
}
