...
import { simhw_short_name, simhw_sim_signal, simhw_sim_signals } from "../sim_hw/sim_hw_index.js";
...
import { jit_fire_dep, jit_fire_order } from "../sim_hw/sim_hw_signal.js";
...
export function wepsim_update_signal_dialog_title(key) {
    ...
    return wepsim_config_dialog_dropdown("success", b_btns, 'var shval = $(\'#ask_shard\').val(); ' +
        'var shkey = $(\'#ask_skey\').val(); ' +
        'ws.update_signal_loadhelp(\'#help2\', shval, shkey);"');
}
...
export function wepsim_update_signal_with_value(key, value) {
    ...
    // add if recording
    simcore_record_append_new('Update signal ' + key + ' with value ' + value, 'ws.wepsim_update_signal_with_value("' + key + '", ' + value + ');\n');
}
