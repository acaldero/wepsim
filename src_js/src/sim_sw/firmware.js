 ...
 *
 */
import { firm_metadata_write, firm_metadata_read } from "./firmware/firm_metadata.js";
import { firm_instruction_write, firm_instruction_read } from "./firmware/firm_instruction.js";
import { firm_registers_write, firm_registers_read } from "./firmware/firm_registers.js";
import { firm_pseudoinstructions_write, firm_pseudoinstructions_read } from "./firmware/firm_pseudoinstructions.js";
import { simhw_sim_ctrlStates_get, simhw_internalState } from "../sim_hw/sim_hw_index.js";
import { frm_nextToken, frm_isToken, frm_langError } from "./firmware/lexical.js";
...
export function loadFirmware(text) {
    ...
    if (context.stackRegister == null) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'SP NOT DEFINED'));
    }
    ...
    for (i = 0; i < context.instrucciones.length; i++) {
        if (context.instrucciones[i].name == "begin") {
            ...
            if (found === false) {
                return frm_langError(context, i18n_get_TagFor('compiler', 'NO LABEL FETCH'));
            }
        }
    }
    if (found === false) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'NO LABEL BEGIN'));
    }
    ...
    if (context.labelsNotFound.length > 0) {
        for (i = 0; i < context.labelsNotFound.length; i++) {
            ...
            if (labelsFounded == 0) {
                ...
                return frm_langError(context, i18n_get_TagFor('compiler', 'NO LABEL MADDR') +
    ...
    if (mk_native != "") {
        // eval(mk_native) ;
        let eval_executor = new Function('simcore_native_get_signal', 'simcore_native_set_signal', 'simcore_native_get_value', 'simcore_native_set_value', 'simcore_native_get_fields', 'simcore_native_get_field_from_ir', 'simcore_native_deco', 'simcore_native_go_maddr', 'simcore_native_go_opcode', 'simcore_native_go_instruction', 'context', mk_native);
        eval_executor(simcore_native_get_signal, simcore_native_set_signal, simcore_native_get_value, simcore_native_set_value, simcore_native_get_fields, simcore_native_get_field_from_ir, simcore_native_deco, simcore_native_go_maddr, simcore_native_go_opcode, simcore_native_go_instruction, context);
    }
    // oc_eoc_hash
    var fioc = 0;
    var fieoc = 0;
    ...
    // revlabels
    context.revlabels = {};
    for (var key in context.instrucciones) {
