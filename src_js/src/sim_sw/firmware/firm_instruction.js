...
import { firm_mcode_write, read_native, firm_mcode_signals_read } from "./firm_mcode.js";
import { frm_getToken, frm_langError, frm_nextToken, frm_isToken } from "./lexical.js";
...
export function firm_instruction_read(context, xr_info, all_ones_oc) {
    ...
    if (instruccionAux.name.match(re_name)[0] != instruccionAux.name) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'INS. NAME') +
            "'" + instruccionAux.name + "' " +
    ...
    while (!frm_isToken(context, "{")) {
        ...
        if (!frm_isToken(context, ",") && !frm_isToken(context, "(") && !frm_isToken(context, ")")) {
            ...
            if (instruccionAux.numeroCampos > 100) {
                return frm_langError(context, i18n_get_TagFor('compiler', 'MORE 100 FIELDS'));
            }
            if (auxValue == "co") {
                return frm_langError(context, i18n_get_TagFor('compiler', 'CO AS FIELD NAME'));
            }
            if (auxValue == "nwords") {
                return frm_langError(context, i18n_get_TagFor('compiler', 'NW AS FIELD NAME'));
            }
        }
        // match optional "(" FIELD ")"
        if (frm_isToken(context, "(")) {
            ...
            if ((typeof instruccionAux.oc != "undefined") &&
            ...
            if (!frm_isToken(context, ",") && !frm_isToken(context, "(") && !frm_isToken(context, ")")) {
                ...
                return frm_langError(context, i18n_get_TagFor('compiler', 'MISSING TOKEN ON') +
                    "'" + current_oc + "'");
            }
            if (frm_isToken(context, ")")) {
                firma = firma + ')';
                ...
                return frm_langError(context, i18n_get_TagFor('compiler', 'MISSING ) ON') +
                    "'" + current_oc + "'");
            }
        }
        firma = firma + ',';
    ...
    if (true == instruccionAux.is_native)
        ...
        ret = firm_mcode_signals_read(context);
    ...
    // *}*
    if (!frm_isToken(context, "}")) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'CLOSE BRACE NOT FOUND'));
    }
    frm_nextToken(context);
    return {};
}
