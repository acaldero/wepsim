...
export function firm_pseudoinstructions_write(context) {
    ...
    for (var ie = 0; ie < context.pseudoInstructions.length; ie++) {
        ...
        o += '\n';
        if ((typeof elto.initial != "undefined") &&
...
export function firm_pseudoinstructions_read(context) {
    ...
    while (!frm_isToken(context, "}")) {
        ...
        while (!frm_isToken(context, "{")) {
            ...
            // name*=*type
            if (!frm_isToken(context, "=")) {
                return frm_langError(context, i18n_get_TagFor('compiler', 'EQUAL NOT FOUND') + ' (for name=type)');
            }
            // name=*type*
            frm_nextToken(context);
            pseudoFieldAux.type += frm_getToken(context).replace("num", "imm");
            if (["reg", "inm", "imm", "addr", "address"].includes(pseudoFieldAux.type) == false) {
                return frm_langError(context, i18n_get_TagFor('compiler', 'INVALID PARAMETER') +
                    pseudoFieldAux.type + '.' +
        ...
        while (!frm_isToken(context, "}")) {
            ...
            if (inStart == 0) {
                if (typeof hash_inst_name[tok] == "undefined") {
                    return frm_langError(context, i18n_get_TagFor('compiler', 'UNDEF. INSTR.') +
                        "'" + tok + "'");
                }
            }
            if (tok == ";")
