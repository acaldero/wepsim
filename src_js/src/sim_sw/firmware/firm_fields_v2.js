...
import { frm_langError, frm_nextToken, frm_isToken, frm_getToken, frm_getTokenType, frm_isToken_arr } from "./lexical.js";
    ...
    for (var j = 0; j < elto_fields.length; j++) {
        ...
        for (var k = 0; k < elto_fields[j].bits_start.length; k++) {
            if (elto_fields[j].bits_start[k] != elto_fields[j].bits_stop[k])
                ...
                o += elto_fields[j].bits_start[k]; // 19
...
export function firm_instruction_check_oc(context, instruccionAux, xr_info, all_ones_oc) {
    // semantic check: valid value
    if (instruccionAux.oc.match("[01]*")[0] != instruccionAux.oc) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'INCORRECT OC BIN.') +
            "'" + instruccionAux.oc + "'");
    }
    ...
    if (instruccionAux.oc != all_ones_oc_len) {
        if ((typeof context.oc_eoc[instruccionAux.oc] !== "undefined") &&
            ...
            return frm_langError(context, i18n_get_TagFor('compiler', 'OC ALREADY USED') +
    ...
    for (var i = 0; i < instruccionAux.fields_all.length; i++) {
        for (var j = 0; j < instruccionAux.fields_all[i].bits_stop.length; j++) {
            if (instruccionAux.fields_all[i].type != "oc")
                continue;
    ...
    if ((instruccionAux["is_native"] === false) &&
        ...
        return frm_langError(context, i18n_get_TagFor('compiler', 'BAD OC BIN. LEN.') +
            " at '" + instruccionAux.name + "'");
    }
    // ok by now
    return {};
}
export function firm_instruction_check_eoc(context, instruccionAux, xr_info, all_ones_oc) {
    // semantic check: valid value
    if (instruccionAux.eoc.match("[01]*")[0] != instruccionAux.eoc) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'INCORRECT EOC BIN.') +
            "'" + instruccionAux.eoc + "'");
    }
    // semantic check: 'oc' is not already used
    var all_ones_oc_len = "".padStart(instruccionAux.oc.length, "1");
    ...
    if (instruccionAux.oc != all_ones_oc_len) {
        ...
        if ((context.oc_eoc[instruccionAux.oc].eoc != null) &&
            ...
            return frm_langError(context, i18n_get_TagFor('compiler', 'OC+EOC ALREADY USED') +
    ...
    for (var i = 0; i < instruccionAux.fields_all.length; i++) {
        for (var j = 0; j < instruccionAux.fields_all[i].bits_stop.length; j++) {
            if (instruccionAux.fields_all[i].type != "eoc")
                continue;
            nbits_field = Math.abs(parseInt(instruccionAux.fields_all[i].bits_stop[j]) - parseInt(instruccionAux.fields_all[i].bits_start[j])) + 1;
    ...
    if ((instruccionAux["is_native"] === false) &&
        ...
        return frm_langError(context, i18n_get_TagFor('compiler', 'BAD EOC BIN. LEN.') +
            " at '" + instruccionAux.name + "'");
    }
    // ok by now
    return {};
...
export function firm_instruction_keynumber_read(context, instruccionAux) {
    var ret = {};
    // li reg val {
    //             ...
    //             *key=000000*,
    ...
    if (!frm_isToken(context, "=")) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'EQUAL NOT FOUND'));
    }
    frm_nextToken(context);
...
export function firm_instruction_keystring_read(context, instruccionAux) {
    var ret = {};
    // li reg val {
    //             ...
    ...
    if ("STRING" != frm_getTokenType(context)) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'UNKNOWN ESCAPE CHAR') +
...
export function firm_instruction_field_read_v2(context, instruccionAux) {
    ...
    if (!frm_isToken_arr(context, complex_field_list)) {
        ...
        // match mandatory (
        if (!frm_isToken(context, "(")) {
            return frm_langError(context, i18n_get_TagFor('compiler', 'OPEN PAREN. NOT FOUND'));
        }
        frm_nextToken(context);
        ...
        if (start > 32 * parseInt(instruccionAux.nwords) - 1) {
            return frm_langError(context, i18n_get_TagFor('compiler', 'STARTBIT OoR') +
                "'" + frm_getToken(context) + "'");
        }
        frm_nextToken(context);
        // match mandatory :
        if (!frm_isToken(context, ":")) {
            return frm_langError(context, i18n_get_TagFor('compiler', 'COLON NOT FOUND'));
        }
        frm_nextToken(context);
        ...
        if (stop > 32 * parseInt(instruccionAux.nwords)) {
            return frm_langError(context, i18n_get_TagFor('compiler', 'STOPBIT OoR') +
                "'" + frm_getToken(context) + "'");
        }
        frm_nextToken(context);
        // match mandatory )
        if (!frm_isToken(context, ")")) {
            return frm_langError(context, i18n_get_TagFor('compiler', 'CLOSE PAREN. NOT FOUND'));
        }
    }
    else {
        // ...
        ...
        if (frm_isToken(context, "|")) {
            ...
            while (!frm_isToken(context, ")")) {
                ...
                if (bits_aux[0] > 32 * parseInt(instruccionAux.nwords)) {
                    return frm_langError(context, i18n_get_TagFor('compiler', 'BIT OoR') +
                        "'" + frm_getToken(context) + "'");
                }
                frm_nextToken(context);
                if (frm_getToken(context) == ")") {
                ...
                if (!frm_isToken(context, ":") && !frm_isToken(context, "|")) {
                    return frm_langError(context, i18n_get_TagFor('compiler', 'COLON OR PIPE NOT FOUND'));
                }
                if (frm_isToken(context, ":")) {
                    frm_nextToken(context);
    ...
    if (["oc", "eoc"].includes(tmp_fields.type)) {
        ...
        if (index_name == -1) {
            return frm_langError(context, i18n_get_TagFor('compiler', 'UNEXPECTED FIELD') +
                "'" + tmp_name + "'. " +
    ...
    for (i = stop; i <= start; i++) {
        if (typeof instruccionAux.overlapping[i] != "undefined") {
            return frm_langError(context, i18n_get_TagFor('compiler', 'OVERLAPPING FIELD') +
...
export function firm_instruction_read_fields_v2(context, instruccionAux, xr_info, all_ones_oc) {
    ...
    if (1 != oc_inserted) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'NO OC FIELD'));
    }
    // semantic check: number of fields
    if (camposInsertados != instruccionAux.numeroCampos) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'NO FIELD')); // TODO
    }
