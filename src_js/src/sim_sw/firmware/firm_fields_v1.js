...
import { frm_nextToken, frm_isToken, frm_langError, frm_getToken, frm_getTokenType, frm_isToken_arr } from "./lexical.js";
...
export function firm_instruction_co_read(context, instruccionAux, xr_info, all_ones_co) {
    ...
    if ((instruccionAux.oc.match("[01]*")[0] != instruccionAux.oc) ||
        ...
        return frm_langError(context, i18n_get_TagFor('compiler', 'INCORRECT CO BIN.') +
            "'" + instruccionAux.oc + "'");
    }
    ...
    if (instruccionAux.oc != all_ones_oc_len) {
        if ((typeof context.oc_eoc[instruccionAux.oc] !== "undefined") &&
            ...
            return frm_langError(context, i18n_get_TagFor('compiler', 'CO ALREADY USED') +
...
export function firm_instruction_cop_read(context, instruccionAux, all_ones_co) {
    ...
    if (instruccionAux.eoc.match("[01]*")[0] != instruccionAux.eoc) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'INCORRECT COP BIN.') +
            "'" + instruccionAux.eoc + "'");
    }
    ...
    if (instruccionAux.oc != all_ones_co) {
        if ((context.oc_eoc[instruccionAux.oc].eoc != null) &&
            ...
            return frm_langError(context, i18n_get_TagFor('compiler', 'CO+COP ALREADY USED') +
...
export function firm_instruction_field_read(context, instruccionAux, camposInsertados) {
    ...
    // match mandatory ,
    if (!frm_isToken(context, ",")) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'COMMA NOT FOUND'));
    }
    frm_nextToken(context);
    ...
    if (instruccionAux.fields[camposInsertados].type == "address") {
        ...
        if (frm_getToken(context) != "abs" && frm_getToken(context) != "rel") {
            return frm_langError(context, i18n_get_TagFor('compiler', 'INCORRECT ADDRESSING'));
        }
...
export function firm_instruction_read_flexible_fields(context, instruccionAux, xr_info, all_ones_co) {
    ...
    for (var k = 0; k < camposInsertados; k++) {
        ...
        if (start > 32 * parseInt(instruccionAux.nwords) - 1) {
            return frm_langError(instruccionAux.fields[k].context, i18n_get_TagFor('compiler', 'STARTBIT OoR') +
                "'" + start + "'");
        }
        // check stopbit range
        var stop = parseInt(instruccionAux.fields[k].stopbit);
        if (stop > 32 * parseInt(instruccionAux.nwords)) {
            return frm_langError(instruccionAux.fields[k].context, i18n_get_TagFor('compiler', 'STOPBIT OoR') +
                "'" + stop + "'");
        }
        // check overlapping
        for (var o = stop; o <= start; o++) {
            if (typeof instruccionAux.overlapping[o] != "undefined") {
                return frm_langError(instruccionAux.fields[k].context, i18n_get_TagFor('compiler', 'OVERLAPPING FIELD') +
    ...
    if (1 != co_inserted) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'NO CO FIELD'));
    }
    ...
    if ((instruccionAux["is_native"] === false) &&
        ...
        return frm_langError(context, i18n_get_TagFor('compiler', 'BAD COP BIN. LEN.') +
            "'" + frm_getToken(context) + "'");
    }
