...
import { frm_resetComments, frm_isToken, frm_langError, frm_nextToken, frm_getToken, frm_getTokenType, frm_getComments, frm_nextNative } from "./lexical.js";
...
export function firm_mcode_signals_read(context) {
    ...
    if (!frm_isToken(context, "{")) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'OPEN BRACE NOT FOUND'));
    }
    ...
    while (!frm_isToken(context, "}")) {
        ...
        if (!frm_isToken(context, "(")) {
            ...
            if ("TAG" != frm_getTokenType(context)) {
                return frm_langError(context, i18n_get_TagFor('compiler', 'LABEL NOT FOUND') +
                    "'" + newLabelName + "'");
            }
            // semantic check: existing LABEL
            for (var contadorMCAux in context.etiquetas) {
                if (context.etiquetas[contadorMCAux] == newLabelName) {
                    return frm_langError(context, i18n_get_TagFor('compiler', 'REPEATED LABEL') +
            ...
            if (newLabelName.match("[a-zA-Z_0-9]*")[0] != newLabelName) {
                return frm_langError(context, i18n_get_TagFor('compiler', 'INVALID LABEL FORMAT') +
                    "'" + frm_getToken(context) + "'");
            }
            frm_nextToken(context);
        }
        ...
        while (!frm_isToken(context, ")")) {
            ...
            if (typeof simhw_sim_signal(nombre_tok) == "undefined") {
                return frm_langError(context, i18n_get_TagFor('compiler', 'SIGNAL NOT EXISTS') +
                    "'" + nombre_tok + "'");
            }
            // semantic check: signal id can be used
            if (typeof simhw_sim_signal(nombre_tok).forbidden != "undefined") {
                return frm_langError(context, nombre_tok + ' ' + i18n_get_TagFor('compiler', 'SIGNAL NO DIRECTLY'));
            }
            ...
            if (frm_isToken(context, "=")) {
                ...
                if (frm_getToken(context).match("[01]*")[0] != frm_getToken(context)) {
                    return frm_langError(context, i18n_get_TagFor('compiler', 'INCORRECT BIN. FORMAT') +
                        "'" + frm_getToken(context) + "'");
                }
                // semantic check: value within range
                if (microInstruccionAux[nombre_tok] >= Math.pow(2, simhw_sim_signal(nombre_tok).nbits)) {
                    return frm_langError(context, i18n_get_TagFor('compiler', 'OUT OF RANGE') +
                        "'" + frm_getToken(context) + "'");
                }
                frm_nextToken(context);
            }
    ...
    if (microprograma.length === 0) {
        return frm_langError(context, i18n_get_TagFor('compiler', 'EMPTY MICROCODE'));
    }
    // match mandatory }
    frm_nextToken(context);
    return {
