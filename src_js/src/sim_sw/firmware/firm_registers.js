...
import { frm_nextToken, frm_isToken, frm_getToken, frm_langError } from "./lexical.js";
...
export function firm_registers_read(context) {
    ...
    while (!frm_isToken(context, "}")) {
        ...
        if (!frm_isToken(context, "(")) {
            ...
            frm_nextToken(context);
            if (frm_isToken(context, ")")) {
                return frm_langError(context, i18n_get_TagFor('compiler', 'EMPTY NAME LIST'));
            }
            // 1=(ra*, x1*),
            rf_item.registers[nombre_reg] = [];
            while (!frm_isToken(context, ")")) {
        ...
        if (frm_isToken(context, "(")) {
            if (context.stackRegister != null) {
                return frm_langError(context, i18n_get_TagFor('compiler', 'DUPLICATE SP'));
            }
            frm_nextToken(context);
            if (!frm_isToken(context, "stack_pointer")) {
                return frm_langError(context, i18n_get_TagFor('compiler', 'NO SP'));
            }
