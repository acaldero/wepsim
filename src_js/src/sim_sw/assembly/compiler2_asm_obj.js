 ...
 *
 */
import { WORD_BYTES, WORD_LENGTH, BYTE_LENGTH } from "../assembly/datatypes.js";
import { sim_segments } from "./memory_segments.js";
import { dt_get_decimal_value, isDecimal, dt_get_imm_value, isFloat, dt_binary2format } from "./datatypes.js";
import { asm_setLabelContext, asm_langError, asm_getToken, asm_nextToken, wsasm_isEndOfFile, asm_getComments, asm_resetComments, asm_getTokenType, asm_getLabelContext } from "./lexical.js";
import { i18n_get_TagFor } from "../../wepsim_i18n/i18n.js";
import { base_replaceAll } from "../../sim_core/sim_core_ctrl.js";
import { wsasm_is_directive_segment, wsasm_is_directive_datatype, wsasm_has_datatype_attr, wsasm_get_datatype_size, wsasm_is_directive } from "./directives.js";
import { decimal2binary, float2binary } from "../../sim_core/sim_core_ui.js";
...
export function wsasm_new_objElto(base_elto) {
    var elto = {
...
export function wsasm_make_signature_user(elto, use_as_around) {
...
export function wsasm_get_sel_valbin(value, start_bit, stop_bit) {
    ...
    if (valbin < 0)
        ...
        valbin = valbin.toString(2).padStart(WORD_LENGTH, '0');
    valbin = valbin.substring(sel_start, sel_stop + 1); // [0:19], [20,31]
    return valbin;
}
export function wsasm_eltoError(context, elto, msg, msgOrigin) {
...
export function wsasm_get_similar_candidates(context, elto) {
    ...
    for (let key in context.firmware) {
        if ((key.includes(elto.value.instruction)) || (elto.value.instruction.includes(key))) { // "sli" == "li", "li" == "li", ...
...
export function wsasm_is_ValidTag(tag) {
    var tg = tag.trim();
    if ("" == tg) {
        return false;
    }
...
export function wsasm_order2index_startstop(start_bit, stop_bit) {
...
export function wsasm_src2obj_data(context, ret) {
    ...
    while ((!wsasm_is_directive_segment(asm_getToken(context))) && // NOT .data/...
        ...
        possible_tag = "";
        while ((!wsasm_is_directive_datatype(asm_getToken(context))) && // NOT .data/...
            (!wsasm_isEndOfFile(context)) // NOT end-of-file
        ) {
            // tagX
            ...
            if ("TAG" != asm_getTokenType(context)) {
                ...
                return asm_langError(context, i18n_get_TagFor('compiler', 'NO TAG OR DIRECTIVE') +
                    "'" + possible_tag + "'", 'no tag/direc. ' + possible_tag);
            }
            ...
            if (wsasm_is_ValidTag(tag) == false) {
                return asm_langError(context, i18n_get_TagFor('compiler', 'INVALID TAG FORMAT') +
                    "'" + tag + "'", 'invalid tag ' + tag);
            }
            if (context.firmware[tag]) {
                return asm_langError(context, i18n_get_TagFor('compiler', 'TAG OR INSTRUCTION') +
                    "'" + tag + "'", 'bad tag name ' + tag);
            }
            if (typeof ret.labels_asm[tag] != "undefined") {
                return asm_langError(context, i18n_get_TagFor('compiler', 'REPEATED TAG') +
                    "'" + tag + "'", 'repeated tag ' + tag);
            }
            // Store tag
        ...
        if (wsasm_has_datatype_attr(elto.datatype, "numeric")) {
            ...
            while ((wsasm_is_directive(asm_getToken(context)) == false) &&
                ...
                if ((ret1.isDecimal == false) && (ret1.isFloat == false)) {
                    ...
                    if (".word" !== elto.datatype) {
                        return asm_langError(context, i18n_get_TagFor('compiler', 'NO NUMERIC DATATYPE') +
                            "'" + possible_value + "'", 'no num. ' + possible_value);
                    }
                    // CHECK valid label
                    if (wsasm_is_ValidTag(possible_value) == false) {
                        return asm_langError(context, i18n_get_TagFor('compiler', 'INVALID TAG FORMAT') +
                            "'" + possible_value + "'", 'invalid tag ' + possible_value);
                    }
                    if (context.firmware[possible_value]) {
                        return asm_langError(context, i18n_get_TagFor('compiler', 'TAG OR INSTRUCTION') +
                            "'" + possible_value + "'", 'bad tag name ' + possible_value);
                    }
                    ...
                    if (ret1.isDecimal == true)
                        ...
                        a = float2binary(number, elto.byte_size * BYTE_LENGTH);
                    num_bits = a[0];
                    free_space = a[1];
                    // CHECK size
                    if (free_space < 0) {
                        return asm_langError(context, i18n_get_TagFor('compiler', 'EXPECTED VALUE') + elto.datatype +
                            "' (" + elto.byte_size * BYTE_LENGTH + " bits), " +
                            i18n_get_TagFor('compiler', 'BUT INSERTED') + possible_value +
                            "' (" + num_bits.length + " bits) " +
                            i18n_get_TagFor('compiler', 'INSTEAD'), 'possible_value of ' + num_bits.length + 'bits');
                    }
                }
                // Add ELTO
                elto.seg_name = seg_name;
            ...
            if (ret1.isDecimal == false) {
                return asm_langError(context, i18n_get_TagFor('compiler', 'NO NUMBER OF BYTES') +
                    "'" + possible_value + "'", 'no number ' + possible_value);
            }
            if (possible_value < 0) {
                return asm_langError(context, i18n_get_TagFor('compiler', 'NO POSITIVE NUMBER') +
                    "'" + possible_value + "'", 'no positive ' + possible_value);
            }
            var byte_val = '0x0';
            ...
            if ((ret1.isDecimal == false) && (possible_value >= 0)) {
                return asm_langError(context, i18n_get_TagFor('compiler', 'INVALID ALIGN VALUE') +
                    "'" + possible_value + "'. " +
                    i18n_get_TagFor('compiler', 'REMEMBER ALIGN VAL'), 'bad align value ' + possible_value);
            }
            // Calculate offset
            ...
            while (!wsasm_is_directive(asm_getToken(context)) && !wsasm_isEndOfFile(context)) {
                // CHECK string
                if ("\"" !== possible_value[0]) {
                    return asm_langError(context, i18n_get_TagFor('compiler', 'NO QUOTATION MARKS') +
                        "'" + possible_value + "'", 'no quotation marks');
                }
                if ("\"" !== possible_value[possible_value.length - 1]) {
                    return asm_langError(context, i18n_get_TagFor('compiler', 'NOT CLOSED STRING'), 'no closed string');
                }
                if ("" == possible_value) {
                    return asm_langError(context, i18n_get_TagFor('compiler', 'NOT CLOSED STRING'), 'no closed string');
                }
            ...
            // CHECK datatype
            return asm_langError(context, i18n_get_TagFor('compiler', 'UNEXPECTED DATATYPE') +
                "'" + elto.datatype + "'", 'bad datatype ' + elto.datatype);
        }
    }
    // Return ret
    return ret;
}
export function wsasm_encode_field(arr_encoded, value, start_bit, stop_bit) {
    var val_i = 0;
    for (let m = 0; m <= start_bit.length; m++) {
...
export function wsasm_encode_instruction(context, ret, elto, candidate) {
    ...
    wsasm_encode_field(arr_encoded, candidate.oc.value, candidate.oc.asm_start_bit, candidate.oc.asm_stop_bit);
    wsasm_encode_field(arr_encoded, candidate.eoc.value, candidate.eoc.asm_start_bit, candidate.eoc.asm_stop_bit);
    ...
    for (let j = 0; j < candidate.fields.length; j++) {
        ...
        if (["imm", "(imm)", "address", "(address)"].includes(elto.value.signature_type_arr[j + 1])) {
            ...
            value = a[0];
            if (a[1] < 0) {
                return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'EXPECTED VALUE') + " immediate" +
                    " (" + n_bits + " bits), " +
                    i18n_get_TagFor('compiler', 'BUT INSERTED') + elto.value.fields[i - 1] +
                    " (" + value.length + " bits) " +
                    i18n_get_TagFor('compiler', 'INSTEAD'), elto.value.fields[i - 1] + ' of ' + value.length + ' bits');
            }
...
export function wsasm_src2obj_text_getDistance(elto_firm_reference_i, elto_value) {
...
export function wsasm_src2obj_text_instr_op_match(context, ret, elto, atom, parentheses) {
    ...
    if ((ret1.isDecimal) || (ret1.isFloat)) {
        ...
        if (ret1.isDecimal)
            ...
            a = float2binary(ret1.number, elto.byte_size * BYTE_LENGTH);
        if (parentheses) {
            elto.value.fields.push('(' + atom + ')');
...
export function wsasm_src2obj_text_elto_field_sel(context, ret, elto, pseudo_context) {
    ...
    if ('(' != opx) {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'OPEN PAREN. NOT FOUND'), 'missing open paren.');
    }
    // sel (*31*,  12 ,  label )
    sel.stop = wsasm_src2obj_text_ops_getAtom(context, pseudo_context);
    sel.stop = parseInt(sel.stop);
    if (isNaN(sel.stop)) {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'NO POSITIVE NUMBER') + sel.stop, 'not positive ' + sel.stop);
    }
    // sel ( 31*,* 12 ,  label )
    opx = wsasm_src2obj_text_ops_getAtom(context, pseudo_context);
    if (',' != opx) {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'COMMA NOT FOUND'), 'comma not found');
    }
    // sel ( 31 , *12*,  label )
    sel.start = wsasm_src2obj_text_ops_getAtom(context, pseudo_context);
    sel.start = parseInt(sel.start);
    if (isNaN(sel.start)) {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'NO POSITIVE NUMBER') + sel.start, 'not positive ' + sel.start);
    }
    // sel ( 31 ,  12*,* label )
    opx = wsasm_src2obj_text_ops_getAtom(context, pseudo_context);
    if (',' != opx) {
    ...
    opx = wsasm_src2obj_text_ops_getAtom(context, pseudo_context);
    if (')' != opx) {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'CLOSE PAREN. NOT FOUND'), 'no close paren.');
    }
    // check if sel.label is number or tag...
    var a = dt_get_imm_value(sel.label);
    if (a.isDecimal) {
        ...
        if (wsasm_is_ValidTag(sel.label) == false) {
            return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'LABEL NOT DEFINED') + ": '" + sel.label + "'", 'no label ' + sel.label);
        }
...
export function wsasm_src2obj_text_elto_field_abs(context, ret, elto, pseudo_context, sel) {
    ...
    opx = wsasm_src2obj_text_ops_getAtom(context, pseudo_context);
    if ('(' != opx) {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'OPEN PAREN. NOT FOUND'), 'no open paren.');
    }
    // %lo ( *label* )
    sel.label = wsasm_src2obj_text_ops_getAtom(context, pseudo_context);
    // %lo ( label*)*
    opx = wsasm_src2obj_text_ops_getAtom(context, pseudo_context);
    if (')' != opx) {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'CLOSE PAREN. NOT FOUND'), 'no open paren.');
    }
    // check if sel.label is number or tag...
...
export function wsasm_src2obj_text_elto_fields(context, ret, elto, pseudo_context) {
    ...
    while ((opx != '') && (elto.value.fields.length < 100)) {
        ...
        if (',' == opx) {
            ...
            return asm_langError(context, i18n_get_TagFor('compiler', 'COMMA NOT FOUND'), 'comma not found');
        }
    }
    // CHECK: More than 100 fields? really? umm, might be an error...
    if (elto.value.fields.length > 100) {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'NOT MATCH FORMAT') + ".<br>" +
            i18n_get_TagFor('compiler', 'REMEMBER FORMAT USED') + " '" + elto.source + "'.<br>" +
            i18n_get_TagFor('compiler', 'CHECK MICROCODE'), 'more 100 fields?');
    }
...
export function wsasm_find_candidate_and_encode(context, ret, elto) {
...
export function wsasm_src2obj_text(context, ret) {
    ...
    while ((!wsasm_is_directive_segment(asm_getToken(context))) && // NOT .data/...
        ...
        // tagX
        possible_tag = asm_getToken(context);
        while ((typeof context.firmware[possible_tag.toLowerCase()] === "undefined") && // NOT instruction
            (!wsasm_isEndOfFile(context)) // NOT end-of-file
        ) {
            // CHECK tag
            if ("TAG" != asm_getTokenType(context)) {
                ...
                return asm_langError(context, i18n_get_TagFor('compiler', 'NO TAG, DIR OR INS') +
                    "'" + possible_tag + "'", 'no tag, instr. or direc.');
            }
            tag = possible_tag.substring(0, possible_tag.length - 1);
            // CHECK valid tag
            if (wsasm_is_ValidTag(tag) == false) {
...
export function wsasm_src2obj_binary(context, ret) {
    ...
    while ((!wsasm_is_directive_segment(asm_getToken(context))) && // NOT .data/...
        ...
        // tagX
        possible_tag = asm_getToken(context);
        while ((possible_tag.endsWith(':')) && // It looks like a tag AND
            (!wsasm_isEndOfFile(context)) // NOT end-of-file
        ) {
            // CHECK tag
            if ("TAG" != asm_getTokenType(context)) {
                ...
                return asm_langError(context, i18n_get_TagFor('compiler', 'NO TAG, DIR OR INS') +
                    "'" + possible_tag + "'", 'no tag, instr. or direc.' + possible_tag);
            }
            tag = possible_tag.substring(0, possible_tag.length - 1);
            // CHECK valid tag
            if (wsasm_is_ValidTag(tag) == false) {
...
export function wsasm_src2obj_helper(context, ret) {
    ...
    while (wsasm_isEndOfFile(context) == false) {
        ...
        if (typeof ret.seg[segname] === "undefined") {
            return asm_langError(context, i18n_get_TagFor('compiler', 'INVALID SEGMENT NAME') +
                "'" + segname + "'", 'invalid seg. ' + segname);
        }
...
export function wsasm_get_label_value(context, ret, elto, label) {
    ...
    if (value_arr.length < 5) {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'LABEL NOT DEFINED') + ": '" + label + "'", 'no label ' + label);
    }
    // Examples:
    // ['', 'w1', '12', '31', '',    '']
    // ['', 'w1', '12', '31', '_pc', '']
    ...
    if (typeof value === "undefined") {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'LABEL NOT DEFINED') + ": '" + sel_label + "'", 'no label ' + sel_label);
    }
...
export function wsasm_resolve_labels_elto(context, ret, elto) {
    ...
    for (let j = 0; j < elto.pending.length; j++) {
        ...
        if (value.length > elto.pending[j].n_bits) {
            return wsasm_eltoError(context, elto, "'" + elto.pending[j].label + "'" +
                i18n_get_TagFor('compiler', 'NEEDS') +
                value.length +
                i18n_get_TagFor('compiler', 'SPACE FOR # BITS') +
                elto.pending[j].n_bits + " " +
                i18n_get_TagFor('compiler', 'BITS'), '# bits for ' + elto.pending[j].label);
        }
        // update elto.binary
 ...
 *  Public API
 */
export function wsasm_prepare_source(context, asm_source) {
    // Check arguments
...
export function wsasm_src2obj(context) {
    // Check arguments
    if (typeof context == "undefined") {
    ...
    if (20 == r) {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'CHECK MICROCODE') +
            " because it might be a circular definition: pseudo <-> pseudo", 'circular definition pseudo<->pseudo');
    }
    ...
    if (ret.text_found) {
        if ((typeof ret.labels_asm["main"] === "undefined") &&
            ...
            return asm_langError(context, i18n_get_TagFor('compiler', 'NO MAIN OR KMAIN'), 'no main/kmain');
        }
    }
    return ret;
}
export function wsasm_obj2src(context, ret, options) {
    var o = '';
    var elto = null;
    var curr_segment = '';
    // check params
    if (typeof ret.obj == "undefined") {
        return wsasm_eltoError(context, elto, i18n_get_TagFor('compiler', 'EMPTY OBJECT CODE'), 'empty object code');
    }
    // prepare options...
