 ...
 *
 */
import { get_var, vue_observable_ifnotjetdone, vue_applyBinding } from "../sim_core/sim_core_values.js";
...
export function wepsim_show_cache_content(level, memory) {
    ...
    if ("" == o1)
        ...
        o1 = "<div class='table-responsive'>" + o1 + "</div>";
    // return content
    return o1;
}
...
export function wepsim_show_cache_vueinit(level, memory) {
     ...
     *       "  </li>\n" ;
     */
    vue_applyBinding(memory.stats.n_access, p1 + 'hitratio', function (value) {
        var hit_ratio = 0.0;
    ...
    vue_applyBinding(memory.stats.n_access, p1 + 'missratio', function (value) {
        var miss_ratio = 0.0;
    ...
    vue_applyBinding(memory.stats.last_r_w, p1 + 'last_r_w', function (value) { return value; });
    memory.stats.last_addr = vue_observable_ifnotjetdone(memory.stats.last_addr);
    vue_applyBinding(memory.stats.last_addr, p1 + 'last_addr', function (value) { return '0x' + value.toString(16); });
    memory.stats.last_h_m = vue_observable_ifnotjetdone(memory.stats.last_h_m);
    vue_applyBinding(memory.stats.last_h_m, p1 + 'lhm_1', function (value) {
        if (value != '')
            return " was a ";
        else
            return "";
    });
    vue_applyBinding(memory.stats.last_h_m, p1 + 'lhm_2', function (value) {
        return value;
    });
    /*
 *	// full-associative
 ...
 *   // set associative
    ...
    vue_applyBinding(memory.stats.last_parts.tag, p1 + 'lp_tag', function (value) {
    ...
    vue_applyBinding(memory.stats.last_parts.set, p1 + 'lp_set', function (value) {
    ...
    vue_applyBinding(memory.stats.last_parts.offset, p1 + 'lp_off', function (value) {
