 ...
 *
 */
import { vue_observable_ifnotjetdone, vue_applyBinding } from "../sim_core/sim_core_values.js";
...
export class ws_io_config extends ws_uielto {
    ...
    render_populate() {
        ...
        for (i = 0; i < curr_iointfactory.length; i++) {
            ...
            vue_applyBinding(curr_iointfactory[i].period, '#int' + i + '_per', function (value) { return value; });
            // probability
            curr_iointfactory[i].probability = vue_observable_ifnotjetdone(curr_iointfactory[i].probability);
            vue_applyBinding(curr_iointfactory[i].probability, '#int' + i + '_pro', function (value) { return value; });
        }
    }
}
