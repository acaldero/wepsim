...
import { simhw_sim_states, simhw_sim_state_getref, simhw_internalState } from "../sim_hw/sim_hw_index.js";
import { set_value, update_value, get_value, vue_observable_ifnotjetdone, vue_applyBinding } from "../sim_core/sim_core_values.js";
...
export function quick_config_rf_register_format() {
    ...
    for (var i = 0; i < r_formats.length; i++) {
        ...
        o1 += quickcfg_html_btn(r_formats[i].label2, "ws.update_cfg(\"RF_display_format\", " +
            "           \"" + r_formats[i].format2 + "\");" +
            "ws.wepsim_refresh_registers();", r_formats[i].colwidth);
    }
    return o1;
}
export function quick_config_rf_register_names() {
    ...
    // make menu
    o2 += quickcfg_html_btnreg('R10', "ws.update_cfg(\"RF_display_name\", \"numerical\");" +
        "ws.wepsim_show_rf_names();", 'col-6');
    if (logical_defined.length == 0)
        o2 += "<div class='col-6 p-1'></div>";
    else
        o2 += quickcfg_html_btnreg(logical_defined.join('|'), "ws.update_cfg(\"RF_display_name\", \"logical\");" +
            "ws.wepsim_show_rf_names();", 'col-6');
    for (var i = 0; i < logical_defined.length; i++) {
        o2 += quickcfg_html_btnreg(logical_defined[i], "ws.update_cfg(\"RF_display_name\", \"logical\");" +
            "ws.wepsim_refresh_rf_names(" + (i + 1) + ");", 'col-6');
    }
    return o2;
}
export function quick_config_rf_register_orientation() {
    var o1 = "";
    o1 = quickcfg_html_onoff('20', 'Register show in horizontal', i18n_get_TagFor('cfg', 'Horizontal'), "  ws.wepsim_config_button_toggle('RF_vertical_pack', false, '20');" +
        "  $('.mp_tooltip').collapse('hide');", "(*) " + i18n_get_TagFor('cfg', 'Vertical'), "  ws.wepsim_config_button_toggle('RF_vertical_pack', true,  '20');" +
...
export function wepsim_init_rf() {
    ...
    if (get_cfg('RF_vertical_pack'))
        ...
        separator_class = "row mp_tooltip collapse";
