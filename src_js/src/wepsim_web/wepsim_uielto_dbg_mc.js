...
import { get_value, vue_observable_ifnotjetdone, vue_applyBinding } from "../sim_core/sim_core_values.js";
import { get_simware } from "../sim_core/sim_adt_core.js";
import { control_memory_set, control_memory_lineToString } from "../sim_core/sim_adt_ctrlmemory.js";
...
import { simhw_sim_ctrlStates_get, simhw_sim_state, simhw_internalState } from "../sim_hw/sim_hw_index.js";
...
export function dbg_set_breakpoint(addr) {
    ...
    if (bp_state && ('instruction' === dbg_level)) {
        wepsim_notify_do_notify('<strong>INFO</strong>', 'Please remember to change configuration to execute at microinstruction level.', 'success', get_cfg('NOTIF_delay'));
    }
    // add if recording
    simcore_record_append_new('Set firmware breakpoint at ' + addr, 'ws.dbg_set_breakpoint(' + addr + ');\n');
}
...
export function wepsim_show_control_memory(memory, index, redraw) {
    ...
    show_control_memory_deferred = setTimeout(function () {
        if (false === redraw)
            ...
            hard_refresh_control_memory(memory, index, redraw);
