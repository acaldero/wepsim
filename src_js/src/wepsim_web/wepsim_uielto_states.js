...
import { simcore_simstate_current2state, simcore_simstate_state2checklist, simcore_simstate_checkreport2html, simcore_simstate_diff_results } from "../sim_core/sim_api_stateshots.js";
...
export function wepsim_dialog_current_state() {
    ...
    simcore_ga('state', 'state.dump', 'state.dump' + '.ci=' + get_value(simhw_sim_state('REG_IR_DECO')) +
        ',ne=' + neltos + ga_str);
}
//
// State: Differences
...
export function wepsim_dialog_check_state(obj_chklst_expected, obj_chklst_current) {
    ...
    if (0 == obj_result.errors)
        ...
        msg = simcore_simstate_checkreport2html(obj_result.result, true);
    $('#check_results1').html(msg);
    // ga
    simcore_ga('state', 'state.chk', 'state.chk' + ',ci=' + get_value(simhw_sim_state('REG_IR_DECO')) +
