...
export function wepsim_activehw(mode) {
    ...
    // info + warning
    wepsim_notify_warning('<strong>WARNING</strong>', 'Please remember the current firmware and assembly might need to be reloaded, ' +
...
export function set_ab_size(diva, divb, new_value) {
    ...
    switch (new_value) {
        ...
        case 13:
            ...
            break;
        case 14:
            $(diva).addClass('col-12');
            $(divb).addClass('col-12 order-2');
            break;
        default:
            $(diva).addClass('col-' + (new_value - 1)); //  1,  2, 3, ...
...
export function wepsim_init_default() {
    // Get URL params
    var url_hash = wepsim_preload_get2hash(window.location, wepsim_init_default_preloadFromHash);
    // 1.- Pre-load defaults
    ...
    installCheck.onsuccess = function () {
        if (!installCheck.result) {
            ...
            installLocFind.onsuccess = function (data) {
                wepsim_notify_success('<h4>Info:<br/></h4>', 'WepSIM was installed.');
            };
            installLocFind.onerror = function () {
                wepsim_notify_error('<h4>Warning:<br/>' + installLocFind.error.name + '</h4>', 'FirefoxOS/KaiOS installation was cancelled.');
            };
        }
    };
}
