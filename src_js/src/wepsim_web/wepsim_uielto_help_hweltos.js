...
import { simhwelto_describe_component_enum_aux, simhwelto_prepare_hash } from "../sim_hw/sim_hw_eltos.js";
    ...
    describe_element(elto_path, array_eltos, hash_eltos, enum_name) {
        ...
        if (array_eltos.length != 0) {
            o = '<tr>' +
                ...
                '<td>' +
                simhwelto_describe_component_enum_aux(elto_path, array_eltos, hash_eltos, enum_name, ',<br>') +
                '</td>' +
                '</tr>';
        }
        return o;
        ...
        for (var b in ahw.elements_hash.by_belong) {
            for (var j = 0; j < ahw.elements_hash.by_belong[b].length; j++) {
                ...
                o1 += '<div class="' + grid + ' d-flex my-2 table-responsive">' +
                    ...
                    '</tr></thead>' +
                    '<tbody>' +
                    this.describe_element(elto_path + ':states:', elto.states_inputs, elto.states, 'Inputs') +
                    this.describe_element(elto_path + ':states:', elto.states_outputs, elto.states, 'Outputs') +
                    this.describe_element(elto_path + ':signals:', elto.signals_inputs, elto.signals, 'Signals') +
                    this.describe_element(elto_path + ':I/O mapping:', elto.states_mapping, elto.states, 'I/O mapping') +
                    '</tbody>' +
                    '</table>' +
                    '</div>';
            }
