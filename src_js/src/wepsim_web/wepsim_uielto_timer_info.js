...
export class ws_io_info extends ws_uielto {
    ...
    render_populate() {
        ...
        for (i = 0; i < curr_iointfactory.length; i++) {
            ...
            vue_applyBinding(curr_iointfactory[i].accumulated, '#int' + i + '_acc', function (value) { return value; });
            curr_iointfactory[i].active = vue_observable_ifnotjetdone(curr_iointfactory[i].active);
            vue_applyBinding(curr_iointfactory[i].active, '#int' + i + '_act', function (value) { return value; });
        }
    }
}
