...
import { simcore_do_nothing_handler, simcore_execute_microprogram, simcore_reset, simcore_check_if_can_continue2, simcore_init, simcore_init_hw, simcore_init_ui, simcore_compile_firmware, simcore_compile_assembly, simcore_assembly_to_binasm, simcore_execute_program } from "../sim_core/sim_api_core.js";
import { get_screen_content, set_screen_content, get_keyboard_content, set_keyboard_content, get_sound_content, set_sound_content } from "../sim_core/sim_core_ui.js";
...
import { simhw_internalState, simhw_sim_ctrlStates_get, simhw_sim_state, simhw_sim_signal, simhw_active } from "../sim_hw/sim_hw_index.js";
import { main_memory_getsrc } from "../sim_core/sim_adt_mainmemory.js";
import { control_memory_lineToString, get_verbal_from_current_mpc } from "../sim_core/sim_adt_ctrlmemory.js";
...
export function wepsim_nodejs_get_instructionset_filtered(data, options) {
    ...
    else if (['DELETE', 'DEL'].includes(filter_action)) {
        for (var i = 0; i < SIMWARE.firmware.length; i++) {
            if (!filter_arr.includes(SIMWARE.firmware[i].name)) {
                filter_firm.push(SIMWARE.firmware[i]);
            }
...
export function wepsim_nodejs_check(data, options) {
    ...
    var ret = wepsim_nodejs_show_checkresults(data.result_ok, false);
    if (false === ret.ok) {
