...
export function cpu_rvpipe_register ( sim_p: Simulator ): Simulator
{
    ...
    sim_p.behaviors[BEHAVIORS.DECO_IMM] = {
        ...
        operation: function (s_expr: string[]): void {
		    ...
		    let   ins = get_value(sim_p.states[ins_src]);
                    const oi  = decode_instruction(sim_p.internal_states.FIRMWARE, sim_p.ctrl_states.ir, ins) as DecodedInstruction;
                    if (oi.oinstruction === null) return;
    ...
    sim_p.behaviors[BEHAVIORS.CPU_RESET] = {
        ...
        operation: function (): void {
            ...
            for (var key in sim_p.states)    if (key != "BR") reset_value(sim_p.states[key]);
